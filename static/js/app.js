...
showMySearches = function() {
	  ...
	  for(var i = 0, l = searchedRoutes.length; i < l; i++) {
		  console.log(searchedRoutes[i].from)
    		html = html + "<tr><td>" + searchedRoutes[i].from + "</td><td>" + searchedRoutes[i].to + "</td><td>" + searchedRoutes[i].date + "</td><td><a url=" + searchedRoutes[i].url + " id=\"searchAgainButton\" class=\"waves-effect waves-light btn\"><i class=\"material-icons center\">replay</i></a></td></tr>";
